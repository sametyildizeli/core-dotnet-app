...
public interface IEFReadRepository<T, TId> : IEFRepository<T, TId> where T : BaseEntity<TId>, new()
{
    ...
    PagedResult<TDto> GetPagedResult<TDto>(int pageNumber, int pageSize, IEnumerable<Expression<Func<T, bool>>>? filters = null, Func<IQueryable<T>, IIncludableQueryable<T, object>>? include = null);
    Task<PagedResult<TDto>> GetPagedResultAsync<TDto>(int pageNumber, int pageSize, IEnumerable<Expression<Func<T, bool>>>? filters = null, Func<IQueryable<T>, IIncludableQueryable<T, object>>? include = null);
    IEnumerable<TDto> GetChaniedFilterResult<TDto>(IEnumerable<Expression<Func<T, bool>>> filters, Func<IQueryable<T>, IIncludableQueryable<T, object>>? include = null);
    Task<IEnumerable<TDto>> GetChaniedFilterResultAsync<TDto>(IEnumerable<Expression<Func<T, bool>>> filters, Func<IQueryable<T>, IIncludableQueryable<T, object>>? include = null);
